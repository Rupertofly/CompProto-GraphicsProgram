...
public class pCircut extends PApplet {
...
public float getPx( int row, int col, int p, int q )
{
  return ( col + ( float )q / 2.0f ) * colWidth;
}

public float getPy( int row, int col, int p, int q )
{
  return ( row + ( float )p / 2.0f ) * rowHeight;
}
...
public void drawTile( int row, int col, int type )
{
  ...
  {
    noStroke();
    ellipse( p11x, p11y, colWidth / 2.0f, rowHeight / 2.0f );
  }

  strokeWeight( rowHeight / 8.0f );
  stroke( 0 );
...
public void keyPressed()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "pCircut" };
    if (passedArgs != null) {
